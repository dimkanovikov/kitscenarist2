...
void ProjectManager::Implementation::updateNavigatorContextMenu(const QModelIndex& _index)
{
    ...
    if (allInRecycleBin) {
        ...
        if (selectedIndexes.size() == 1) {
            ...
            //
            if (_index.isValid() && selectedIndexes.size() == 1
                && isCurrentDocumentExportAvailable) {
                                                    ...
                                                    bool _force)
{
    ...
    dialog->showDialog(
        {}, tr("Do you really want to permanently remove document \"%1\"?").arg(_item->name()),
